...
tr.samp<-readRDS("~/R/Capstone/Sample Data/trsamp.RDS")  # Read in hte corpus of prepared samples


```
####How do we decide on sample size
At what number of samples does the number of 1-grams stop increasing?
...
# Remove any words appearing on Google's list of profane words  
  ...
  #remove stopwords
   x<-tm_map(x, removeWords, stopwords("en"))
    writeLines("Remove stop words", con)
    writeLines(substring(x[[1]]$content,1,600),con)
...
clean.tr.samp <- clean(tr.samp,TRUE)


```

####Blog Data Cleaning Approach
...
Tokenizer_1 <- function(x) NGramTokenizer(x, 
                                        Weka_control(min = 1, 
                                                     max = 1,
                                                     delimiters =" .\n"))
Tokenizer_n <- function(x) NGramTokenizer(x, 
                                        Weka_control(min = 2, 
                                                     max = 5,
                                                     delimiters =" .\n"))
tdm <- removeSparseTerms(TermDocumentMatrix(clean.tr.samp, control = list(tokenize = Tokenizer)),0.9)
tdm2<-as.matrix(tdm)
a<-data.table(row.names(tdm2),rowSums(tdm2))


# freqtable<-data.table(ngram = tdm$dimnames$Terms[tdm$i], Length = stri_count_words(tdm$dimnames$Terms[tdm$i]), Document = tdm$j, Frequency = tdm$v)
# a<-aggregate(freqtable$Frequency, list(freqtable$ngram), sum)
