...
ngramfreq<-data.table(ngram="",probability = 0,frequency= 0)
...
ngramfreq<-rbind(ngramfreq,ngramcoverage(tdm)); rm(tdm)
...
system.time(final<-ngramfreq[,.(Mean.Probability  =  mean(probability), Mean.Frequency = mean(frequency)) ,by = .(ngram)]
)
final<-final[,wordcount := stri_count_words(ngram)]
