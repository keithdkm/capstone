...
tr.samp<-readRDS("~/R/Capstone/Sample Data/trsamp_10_10.RDS")  # Read in hte corpus of prepared samples
...
clean.tr.samp <- clean(tr.samp,FALSE)
...
#Calculate 1-gram word frequencies
tdm <- TermDocumentMatrix(clean.tr.samp, control = list(tokenize = Tokenizer_2 ))
...
word.freq<-word.freq[,coverage:=round(100*cumsum(word.freq$frequency)/word_count,3)]

g<-qplot(x = 1:nrow(word.freq),y = word.freq$coverage,main = "Coverage", xlab = "No. of words in Dictionary", ylab = "Percentage Document Sample Coverage () ",type = "line")
print(g)
dict<-word.freq[coverage<=10,ngram]
