...
# Make all words lower case. We are not going to try and predict proper nouns 
...
writeLines(corp.blog[[1]]$content, "cleantext.txt")
...
freqtable<-data.table(Token     = blog.tdm$dimnames[[1]], 
                      Frequency = blog.tdm$v)

freqwords<-findFreqTerms(blog.tdm,lowfreq=10)
```

#### Benchmarking
Running the 5% **uncleaned** sample of the blog data through the tokenizer for 2grams and 3grams required 2Gb of memory, took about 5 minutes and generated ?? tokens
...
- Source affects probablilties of words
- Sentence position affects probabilities
- What is the probability that a word is located in a particular position 
##Questions 
