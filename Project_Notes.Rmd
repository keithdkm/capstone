...
# file.name<-paste0("Sample Data/trsamp_",n,"_",as.character(size),".RDS")
# 
...
ngramfreq<-data.table(ngram = "")

for (i in 1:n) {
  
file.name<-paste0("Sample Data/trsamp_",i,"_",as.character(size),".RDS")

tr.samp<-readRDS(file.name)  # Read in hte corpus of prepared samples

clean.tr.samp <- clean(tr.samp,FALSE); rm(tr.samp)

tdm <- TermDocumentMatrix(clean.tr.samp, control = list(tokenize = Tokenizer_3 ))

y<-ngramcoverage(tdm); rm(tdm)

ngramfreq <- merge (x = ngramfreq, y = y , by = "ngram", all = TRUE, suffixes = as.character(i) )
