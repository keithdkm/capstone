...
phrase <-function(t.text,n = 1,model = "Interpolate", params = list(l1 = 0.15, l2 = 0.2, l3 = 0.4, l4 = 0.25)) {
  ...
  if (model %in% c("Backoff")) {
    ...
    table.predict<<-  data.table(rbind(
      ...
      trigrams   [target[,.(  v,w)],  ][!(x %in% tags)][,weighted.prob := params$l3* (probability/2^25)][order(-weighted.prob), .(v,w,x, weighted.prob,table = 3, lambda = params$l3)][1:min(.N,10)], 
      bigrams    [target[,.(    w)],  ][!(x %in% tags)][,weighted.prob := params$l2* (probability/2^25)][order(-weighted.prob), .(w,x, weighted.prob,table = 2, lambda = params$l2)][1:min(.N,10)], 
