...
Here are samples from the top, middle and bottom of the unigram,2gram and 3gram list sorted by probability.: 
```{r, echo= FALSE}
final<-final[-grep("<[se]>",final[,ngram],value = FALSE),]
kable(data.frame(ngram = c("Unigram","Bigram","Trigram"), 
              Top = c(paste(final[.(1), ][order(-count)][1:10,ngram],collapse=","), 
                      paste(final[.(2), ][order(-count)][1:10,ngram],collapse=","),
                      paste(final[.(3), ][order(-count)][1:10,ngram],collapse=",")), 
          Middle  = c(paste(final[.(1), ][order(-count)][(final[.(1),.N]/2)+ -5:5,ngram],collapse=","), 
                      paste(final[.(2), ][order(-count)][(final[.(2),.N]/2)+ -5:5,ngram],collapse=","),
                      paste(final[.(3), ][order(-count)][(final[.(3),.N]/2)+ -5:5,ngram],collapse=",")), 
              End = c(paste(final[.(1), ][order(-count)][(final[.(1),.N]) - 0:9, ngram],collapse=","),
                      paste(final[.(2), ][order(-count)][(final[.(2),.N]) - 0:9, ngram],collapse=","),
                      paste(final[.(3), ][order(-count)][(final[.(3),.N]) - 0:9, ngram],collapse=",")) ))

final[.(1),Mean.Probability := count/final[.(1),.N] ] 
final[.(2),Mean.Probability := count/final[.(2),.N] ]
final[.(3),Mean.Probability := count/final[.(3),.N] ]
```
...
Plotting these frequencies for unigrams 
...
```
```{r,, echo = F, cache= TRUE,eval=TRUE}
g<-ggplot(final[.(1),],aes(log10(1:final[.(1),.N]),Cum.Probability))+geom_line()+ggtitle("Cumulative Probability Density for unigrams by  Dictionary Size") + xlab("Log 10(Dictionary Size in Words)") + ylab("Cumulative Probability of Word Inclusion")
print(g)
```
...
```{r, echo = F, cache =TRUE, eval=true}
g<-ggplot(final[wordcount==2,],aes(log10(1:final[wordcount==2,.N]),Cum.Probability))+geom_line()+ggtitle("Cumulative Probability Density for bigrams by  Dictionary Size") + xlab("Log 10(Dictionary Size in Words)") + ylab("Cumulative Probability of Word Inclusion")
print(g)
```

This graph show that a dictionary of `r final[wordcount==2 & Cum.Probability<0.5 ,.N]` bigrams has a 50% chance of any a 2gram word being in the dictionary and that even a dictionary of `r final[wordcount==2 & Cum.Probability<0.9 ,.N]` bigrams only has 90% chance of including the 2gram.  

The results are similar with trigrams. With  `r format(final[.(3),.N],big.mark = ",", scientific = FALSE )` unique ngrams a dictionary with sufficient coverage is going to be too large.

##Conclusion

Given these results for unigrams and bigrams, the intent is to use a set of ngrams that are limited to predicting the most likely 15,000 words from the unigram dictionary.  In order to predict less likely words an additional strategy will be required.  
