...
The results are similar with trigrams. With  `r format(final[wordcount==3,.N],big.mark = ",", scientific = FALSE )` unique ngrams a vocabulary with sufficient coverage is going to be too large.

##Conclusion

The text prediction tool will probably use a trigram model as its basis on a vocabulary of 15,000 words.  The strategies to handle unseen ngrams that are being considered are backoff and smoothing but it has yet to be decided which.  It is intended that the test sample generated in the data cleaning step will be used to assess the accuracy (perplexity) of the possible appraoches.  With respect to the Shiny app, the ideal application will offer a drop down box with three best predictions after the space bar is pressed.  
